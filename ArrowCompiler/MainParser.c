...
arrow_int errorNumber;     /* Run-time error number = 0 by default (ANSI) */

/* External objects */
extern arrow_int syntaxErrorNumber /* number of syntax errors reported by the parser */;
extern arrow_int line; /* source code line number - defined in scanner.c */
...
extern arrow_void startParser(arrow_void);
extern arrow_int startScanner(ReaderPointer sc_buf);

static arrow_void printParserError(arrow_char* fmt, ...);
static arrow_void displayParser(ReaderPointer ptrBuffer);
static arrow_int getParserFileSize(arrow_char* fname);
static arrow_void callGarbageCollector(arrow_void);
...
arrow_int mainParser(arrow_int argc, arrow_char** argv) {

	FILE* fi;       /* input file handle */
	arrow_int loadsize = 0; /*the size of the file loaded in the buffer */
...
arrow_void printParserError(arrow_char* fmt, ...) {

	va_list ap;
	va_start(ap, fmt);

	(arrow_void)vfprintf(stderr, fmt, ap);
	va_end(ap);
...
arrow_int getParserFileSize(arrow_char* fname) {
	FILE* input;
	arrow_int flength;
	input = fopen(fname, "r");
	if (input == NULL) {
...
arrow_void displayParser(ReaderPointer ptrBuffer) {
...
arrow_void callGarbageCollector(arrow_void) {
