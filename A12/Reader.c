...
ReaderPointer readerAddChar(ReaderPointer const readerPointer, arrow_char ch) {
	...
	if(!readerPointer) return NULL;
	if(!readerPointer->content || ch>=NCHAR){
	...
	if (readerPointer->position.wrte * (arrow_int)sizeof(arrow_char) < readerPointer->size) {
		...
		if (newSize < 0 || newSize < readerPointer->size) return NULL;
...
arrow_bool readerSetMark(ReaderPointer const readerPointer, arrow_int mark) {
	if (!readerPointer || mark<0 || mark > readerPointer->position.wrte) return ARROW_FALSE;
	/* Adjust mark */
...
arrow_bool readerRecover(ReaderPointer const readerPointer) {
	...
	/* Recover positions (I'm not sure if this should recover other positions as well)*/
